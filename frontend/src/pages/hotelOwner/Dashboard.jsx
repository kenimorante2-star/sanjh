...
const BACKEND_URL = import.meta.env.VITE_BACKEND_URL;
    ...
    // NEW: States for a generic booking details modal
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {activeWalkInBookings.map((booking) => { // Added defensive check
                                                ...
                                                if (checkOutDateTime < now && booking.status !== 'Checked-Out') {
                                                    // If current date passed check-out date and not yet checked out
                                                    rowBackgroundColorClass = 'bg-red-500'; // Darker red/pink for overdue
                                                } else if (booking.isPaid === 'Fully Paid') {
                                                    rowBackgroundColorClass = 'bg-green-50'; // Light green
                                                } else if (booking.isPaid === 'Partial') {
                                                    rowBackgroundColorClass = 'bg-yellow-50'; // Light yellow
                                                } else if (booking.isPaid === 'Not Paid') {
                                                    rowBackgroundColorClass = 'bg-red-50'; // Light red/pink
                                                }
                                                    ...
                                                    <tr key={booking.id} className={`${rowBackgroundColorClass}`}>
                                                        ...
                                                        <td className="px-4 py-2 whitespace-nowrap text-xs text-gray-900">
                                                            <span className={`px-2 inline-flex text-xs leading-5 rounded-full
                                                                ...
                                                                {booking.isPaid ? booking.isPaid.charAt(0).toUpperCase() + booking.isPaid.slice(1) : 'Not Paid'}
                                                            </span>
                                                        </td>
            ...
            {verifyPaymentFor && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                        ...
                        <p className="mb-2 text-sm">Amount Already Paid: <strong>₱{parseFloat(verifyPaymentFor.amountPaid || 0).toFixed(2)}</strong></p>
                        <p className="mb-4 text-sm">Balance: <strong>₱{parseFloat(verifyPaymentFor.totalPrice - verifyPaymentFor.amountPaid || 0).toFixed(2)}</strong></p>
